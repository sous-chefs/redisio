...
when 'fedora'
  shell = '/bin/sh'
  # this is necessary because selinux by default prevents the homedir
  # from being managed in /var/lib/
  homedir = '/home'
else
  shell = '/bin/sh'
  homedir = '/redis'
...
# Default settings for all redis instances,
# these can be overridden on a per server basis in the 'servers' hash
default['redisio']['default_settings'] = {
  ...
  'shutdown_save'           => false,
  # Defaults to ['900 1','300 10','60 10000'] inside of template.
  # Needed due to lack of hash subtraction
  'save'                    => nil,
  ...
  'cluster-enabled'            => 'no',
  # Defaults to redis instance name inside of template if cluster is enabled.
  'cluster-config-file'        => nil,
  'cluster-node-timeout'       => 5,
  'includes'                   => nil
}

# The default for this is set inside of the "install" recipe.
# This is due to the way deep merge handles arrays
