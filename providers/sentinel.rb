...
def sentinel_name(current)
  suffix = current['name'] || current['port']
  "sentinel_#{suffix}"
end

def piddir(current)
  sentinel_name = sentinel_name(current)
  base_piddir = new_resource.base_piddir
  "#{base_piddir}/#{sentinel_name}"
end
...
def configure_instance(current_instance)
  ...
  configure_logfile(current) unless current['logfile'].nil?
...
def configure_conffile(current)
  ...
  recipe_eval do
    my_sentinel_name = sentinel_name(current)
    full_template_path = "#{current['configdir']}/#{my_sentinel_name}.conf"

    # Lay down the configuration files for the current instance
    template full_template_path do
...
def configure_initd(current)
  recipe_eval do
    my_sentinel_name = sentinel_name(current)
    full_template_path = "/etc/init.d/redis_#{my_sentinel_name}"
...
def configure_upstart(current)
  recipe_eval do
    my_sentinel_name = sentinel_name(current)
    full_template_path = "/etc/init/redis_#{my_sentinel_name}.conf"
